...
A terminal landing merges the recorded review objects, exactly as a plain `knit land` does, and then archives the bundle. `knit land --target <branch>` keeps its narrower meaning at any destination — retarget the recorded reviews to that branch and merge them there — while its terminality still decides whether the bundle is archived. Once a landing run finishes, `knit land --lane <next>` plans the next environment instead of reporting the finished run; a failed or rolled-back run still has to be resumed or rolled back first.
...
By default a deployment watches the repository it deploys. `whenChanged` widens that, because a deployment does not always depend only on its own repository: an image that builds another repository's binary into itself has to redeploy when *that* repository changes, or it quietly ships a stale one. Write `"whenChanged": ["api", "api-client", "shared-tools"]` on the deployment and it runs when any of them changed. A literal `"*"` runs it on every landing, as in `landing.lanes.<name>.branches`. An unknown repository id in `whenChanged` is refused rather than ignored: watching a repository that does not exist means never deploying, and a typo is exactly what that looks like. The deployments a plan leaves out are recorded in its `deploymentsSkipped`, with what each one watches, and printed under `Deployments not run:` — for the same reason lane absences are printed, so a step missing on purpose never reads like one lost to a bug.
