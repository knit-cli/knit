...
src/
  ...
  commands/
    ...
    runtime/          thin adapter for `knit run up/status/down`: translates bundle state into the knit-runtime crate's RuntimeContext
    schema.rs
...
- `providers/` owns the `Forge` trait and its host adapters. `mod.rs` defines the trait, the canonical `PullRequest`/`CheckRun` types, host detection from a remote URL (`for_remote`/`for_repo`/`by_id`), a shared CLI runner, and the provider-agnostic publication helpers. The GitHub, GitLab, Forgejo, and Bitbucket adapters map host JSON onto the canonical types. GitLab and Forgejo retain their `glab`/`tea` workspace paths while REST supplies richer status and artifact-mode operations; Bitbucket is REST-only. Unknown remotes retain the historical GitHub fallback. Provider modules expose small operations; command modules decide workflow policy.
...
Local inspection uses a shared query engine for `knit log` and the compatible `knit history list`. Project queries maintain a disposable SQLite index below `.knit/cache/history/`, incorporating appended events and rebuilding when the source ledger is replaced or rewritten. The JSONL ledger remains authoritative; the cache is never synchronized. Rebuilding from the ledger preserves events whose bundle artifacts no longer exist. Bundle-only inspection can use the same query logic over a local bundle snapshot without a project or a canonical-ledger write.

Queries select events, recorded groups, or bundles before applying the result limit. Repository sets, resolved local views, event kinds, dates, and message patterns compose independently of formatting. Inspection does not implicitly refresh bundle history, run Git fetch, or contact a sync remote. Explicit refresh and synchronization commands change the available history; query-cache maintenance only changes derived local files. See [history queries](history-queries.md) for the CLI contract.
