...
Expected result: `.knit/land-plans/venue-capacity.land.json` lists one `merge_pr` step per published repo. Bare `knit land` creates or shows the plan but does not execute it. Only run `knit land apply` against PRs you are comfortable merging. A successful apply into a terminal destination archives the bundle and removes generated worktrees unless `--keep-worktrees` is passed; an intermediate destination leaves the bundle open. A failed apply writes `.knit/land-runs/<plan-id>-<timestamp>.run.json` and leaves generated worktrees in place; after fixing the failed step, run `knit land resume`.
