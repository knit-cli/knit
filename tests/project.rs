...
fn project_add_infers_nonstandard_cached_remote_default() {
    let root = unique_temp_dir();
    let (remote, acme, _collaborator) = init_remote_repo(&root, "acme");
    let workspace = root.join("workspace");
    fs::create_dir_all(&workspace).unwrap();

    git(&acme, ["branch", "master"]);
    git(&acme, ["checkout", "-b", "stable"]);
    git(&acme, ["push", "--set-upstream", "origin", "stable"]);
    git(&remote, ["symbolic-ref", "HEAD", "refs/heads/stable"]);
    git(&acme, ["remote", "set-head", "origin", "-a"]);

    knit(&workspace, ["init", "demo"]);
    let added = knit(
        &workspace,
        ["project", "add", "acme", acme.to_str().unwrap()],
    );
