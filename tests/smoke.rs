...
fn project_default_repos_start_bundle_without_track() {
    ...
    knit(&workspace, ["project", "init", "demoapp"]);
    knit(
        &workspace,
    ...
    assert!(knit(&workspace, ["project", "list"]).contains("demoapp"));
    assert!(knit(&workspace, ["project", "show"]).contains("\"id\": \"demoapp\""));
    ...
    .unwrap();
    assert_eq!(bundle["projectId"].as_str(), Some("demoapp"));
...
fn project_remove_deletes_template_and_clears_active_project() {
    ...
    let project_path = workspace.join(".knit/projects/demoapp.project.json");
    assert!(project_path.exists());
    let refused = knit_fails(&workspace, ["project", "remove", "demoapp"]);
    assert!(refused.contains("requires --force"));

    let removed = knit(&workspace, ["project", "remove", "demoapp", "--force"]);
...
fn bundle_split_preflights_project_repos_before_creating_bundle() {
    ...
    assert!(split.contains("Project demoapp is missing repo(s) needed for this split: frontend"));
    assert!(!workspace
...
fn project_agents_are_generated_from_project_json() {
    ...
    fs::write(
        ...
        "custom guidance\n\n## Demoapp Knit Project\n\nThat command should add all four Demoapp repos by default:\n\n- `backend`\n\n<!-- BEGIN GLOSS AGENTS -->\nkeep this\n<!-- END GLOSS AGENTS -->\n",
    )
    .unwrap();

    let refresh = knit(&workspace, ["project", "init", "demoapp", "--agents"]);
    assert!(refresh.contains("Project AGENTS.md"));
    let agents = fs::read_to_string(workspace.join("AGENTS.md")).unwrap();
    assert!(agents.contains("custom guidance"));
    assert!(agents.contains("<!-- BEGIN KNIT PROJECT AGENTS: demoapp -->"));
    assert!(agents.contains("That command adds these default repos from the project data:"));
    assert!(agents.contains("- `backend`"));
    assert!(!agents.contains("all four Demoapp repos"));
    assert!(agents.contains("<!-- BEGIN GLOSS AGENTS -->"));
    assert!(
        agents.contains("<!-- END KNIT PROJECT AGENTS: demoapp -->\n<!-- BEGIN GLOSS AGENTS -->")
    );
    ...
    assert_eq!(
        updated
            .matches("<!-- BEGIN KNIT PROJECT AGENTS: demoapp -->")
            .count(),
        1
    );
    ...
    knit(&workspace, ["project", "agents", "demoapp"]);
...
fn remote_config_supports_global_fallback_and_workspace_override() {
    ...
    knit_with_env(&workspace, ["project", "init", "demoapp"], &env);
...
fn project_landing_template_orders_merges_and_runs_deploy_from_base_checkout() {
    ...
    assert_eq!(plan["sourceProjectId"].as_str(), Some("demoapp"));
