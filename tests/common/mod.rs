...
pub fn spawn_fake_knithub_export(bundle_slug: &str, lifecycle_state: &str) -> String {
    ...
    let base_url = format!("http://{}", listener.local_addr().unwrap());
    let body = format!(
        "{{\"data\":{{\"project\":{{\"slug\":\"demoapp\"}},\"knitProject\":null,\"repositories\":[],\"bundles\":[{{\"id\":\"rb-1\",\"slug\":\"{bundle_slug}\",\"lifecycleState\":\"{lifecycle_state}\",\"currentArtifact\":null}}],\"historyEvents\":[]}}}}"
    );
    std::thread::spawn(move || {
        for stream in listener.incoming() {
            let Ok(mut stream) = stream else { continue };
