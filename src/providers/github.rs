...
mod tests {
    ...
    fn treats_checks_permission_errors_as_nonfatal() {
        let err = anyhow::anyhow!(
            "gh pr checks https://github.com/marc-merino/sampleapp-frontend/pull/45 --json name,state,bucket --required --repo Marc-Merino/sampleapp-frontend failed in /tmp: GraphQL: Resource not accessible by personal access token (node.statusCheckRollup.nodes.0.commit.statusCheckRollup)"
        );
