...
/// Whether an event is an obsolete raw-Git `base.commit` row. Direct base
/// branch history is Git's, not Knit's: commits enter the ledger only inside
/// bundle context. Rows of this kind were generated by older versions, so
/// every read path excludes them, append ignores them, and a rebuild drops
/// them while preserving legitimate historical rows.
pub fn is_obsolete_base_commit(event: &HistoryEvent) -> bool {
    event.kind == "base.commit"
}
...
/// preserved, so a rebuild never loses recorded history. Obsolete generated
/// `base.commit` rows are the exception: no bundle artifact can own them, and
/// a rebuild removes them from the ledger.
pub fn rebuild_project_history(root: &Path, project_id: &str) -> Result<RebuildSummary> {
    ...
    for event in existing {
        if is_obsolete_base_commit(&event) {
            // Generated rows no artifact can regenerate are removed, not
            // preserved: they never represented bundle activity.
            continue;
        }
...
pub fn append_history_events(
    ...
    let mut appended = 0;
    for event in events {
        // Obsolete incoming base.commit rows — from an older writer or a
        // sync remote that still holds them — are ignored, never recorded.
        if is_obsolete_base_commit(event) {
            continue;
        }
...
mod tests {
    ...
    /// An obsolete raw-Git row of the kind older versions generated from
    /// configured base branches. Synthetic fixture; no real repository data.
    fn obsolete_base_commit_event(id: &str, sha: &str) -> HistoryEvent {
        serde_json::from_value(serde_json::json!({
            "schemaVersion": HISTORY_EVENT_SCHEMA_VERSION,
            "eventId": id,
            "projectId": "demo",
    ...
    #[test]
    fn append_ignores_obsolete_incoming_base_commit_events() {
        let workspace = RefreshWorkspace::new();
        let appended = append_history_events(
            workspace.root(),
            "demo",
            &[
                obsolete_base_commit_event("khist_base0000000001", "aaaa1"),
                obsolete_base_commit_event("khist_base0000000002", "bbbb2"),
                orphan_event("khist_live0000000001", "alpha-work", "Bundle activity"),
            ],
        )
        .unwrap();
        assert_eq!(appended, 1, "only the bundle row may be appended");
        let text = workspace.ledger_text();
        assert!(!text.contains("base.commit"));
        assert!(text.contains("Bundle activity"));
    }

    #[test]
    fn rebuild_drops_obsolete_base_commit_rows_but_preserves_real_history() {
        let workspace = RefreshWorkspace::new();
        ...
        let summary = rebuild_project_history(workspace.root(), "demo").unwrap();
        let events = load_history_events(workspace.root(), "demo").unwrap();
        assert!(
            !events.iter().any(is_obsolete_base_commit),
            "obsolete generated rows must be removed"
        );
        assert_eq!(
            summary.preserved, 1,
            "only the deleted-bundle row is preserved"
        );
        assert_eq!(
            summary.replaced, 2,
            "created + commit replaced from the artifact"
        );
        assert_eq!(summary.added, 0);
        assert_eq!(count_kind(&events, "alpha-work", "commit.recorded"), 1);
        assert!(events
            .iter()
            .any(|event| event.bundle_id.as_deref() == Some("deleted-work")));
    }
